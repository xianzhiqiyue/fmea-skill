...
接口失配、环境应力失效、老化/磨损失效、误操作/维护错误、检验逃逸。
...
输出必须是覆盖型草稿。除非用户明确要求快速样例,每个 scope 不少于 4 行,
整份 Excel/OpenClaw 草稿不少于 20 行。历史案例不足时,仍需输出带
`needs expert confirmation` 的覆盖补缺行,不能只返回少数命中案例。
